...
shinyServer(function(input, output, session) {
    ...
    output$downloadResLink <- downloadHandler(
        filename=function() {
            paste('pmcERRRes-', Sys.Date(), '.RData', sep='');
        },
        content=function(file) {
            save(pprDldrs=pprDldrs, relations=relations, file=file);
        }
    )
    ...
    observeEvent(input$searchbutton, {
        ...
        if (entitiesInput == '' | length(entitiesInput) == 0) {
            ...
            pprDldrs <<- lapply(seq_along(entitiesList), function(i) {
                ...
                pprDldr <- PaperDownloader(entities=entities,
                                           papersDir='./papers');
                pprDldr <- getPapersIds(pprDldr);
            })

            outtable <- cbind(Entities=entitiesInput,
                '#Papers'=unlist(lapply(
                    pprDldrs, function(pprDldr) length(pprDldr@papers))));
...
showRelation <- function(relations, selectedPaper, output, session) {
    ...
    actLink <- paste('https://www.ncbi.nlm.nih.gov/pmc/articles/PMC',
                     actPaper@id, sep='');
    updateTextInput(session, 'paperLink', value=actLink);

    for (entity in actRel@entities) {
        actRelSents <- gsub(entity, paste('<B><i>', entity, '</i></B>', sep=''),
                            actRelSents, ignore.case=TRUE);
    }

    actRelSents <- paste(actRelSents, collapse=
                    '<br><br><B>------------------------------</B><br><br>');
    output$paperOutput <- renderText({ actRelSents });
}

updateNewSearchPage <- function(entities, session) {
    updateTextAreaInput(session, 'entitiesInput',
                        value=paste(entities, collapse='\n'));
}
...
updateRelationsPage <- function(relations, session) {
    ...
    names(entChoices) <- unlist(lapply(relations,
                               function(x) paste(x@entities, collapse=' | ')));

    updatePapersSelector(relations, entChoices, session);

    updateSelectInput(session, 'entitySelInput', choices=entChoices,
                      selected=entChoices);
